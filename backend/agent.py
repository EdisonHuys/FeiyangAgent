...
"""
    ...
    def quick_confirm(self, payload):
        ...
        quick_prompt = (
            ...
            f"然后判断：原始信号的方向在当前市场条件下是否仍然有效？如果方向相反，应优先选择 reverse（修改挂单方向为新方向继续埋伏），而不是直接 cancel。除非市场极度混乱无明确方向，才选择 cancel。\n\n"
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "判断标准：\n"
                            "- keep：短期走势方向与原始信号方向一致，价格有较大概率向吃单区间移动，继续埋伏。\n"
                            "- reverse（首选）：短期走势方向明确与原始信号方向相反，市场结构已反转，应修改挂单方向为新方向并继续埋伏。\n"
                            "- cancel：短期走势方向极度不确定或市场结构发生破坏性变化（如黑天鹅、重大利空/利多事件），不适合任何方向入场。\n\n"
                            "重要：当短期走势方向与原始信号方向相反时，应优先选择 reverse（修改方向），而不是 cancel。\n"
                            "只有市场极度混乱、没有任何明确方向预期时才选择 cancel。\n\n"
