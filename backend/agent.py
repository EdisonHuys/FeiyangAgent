...
═══════════════════════════════════════
1. 禁止盲目追涨杀跌：原则上 4H 收盘价远离 MA5/MA10 时不追单。但若满足以下任一条件，在严格控制底线止损（多单止损设在关键支撑下方，空单止损设在关键阻力上方）的前提下，允许追单：
   - 强力突破：伴随极强突破信号（如放量突破关键水平阻力位/支撑位，且 4H 级别 OBV 创出近期新高/新低，或 ADX > 30 强势单边）。
   - 均线金叉/站稳反弹追多：当 4H 收盘价从布林下轨附近向上突破且站稳 MA5 与 MA10（或者 4H 级别 MA5 金叉 MA10），且 MACD 负值柱状图持续收窄，只要此时距离上方关键阻力位有足够的盈亏空间（R:R >= 1.5），允许作为 4H 级别底部右侧确认信号进行『合理追多』。
   - 均线死叉/跌破回调追空：当 4H 收盘价从布林上轨附近向下跌破且跌穿 MA5 与 MA10（或者 4H 级别 MA5 死叉 MA10），且 MACD 正值柱状图持续收窄，只要此时距离下方关键支撑位有足够的盈亏空间（R:R >= 1.5），允许作为 4H 级别顶部右侧确认信号进行『合理追空』。
...
- 若 1W 和 1D 方向一致 → 高置信度方向；若矛盾但存在 4H 级别明显的均线金叉/超卖支撑反弹信号 → 允许作为 4H 级别反弹/回调波段操作（此时置信度评分上限为 8 分，必须具备极佳的盈亏比 R:R >= 1.5），防止完全错过大周期冲突但小周期结构清晰的波段机会。
