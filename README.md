...
- **🎯 智能狙击控制台 (Sniper Engine)**：
  ...
  - **双保险自动风控与推损保本**：达到 TP1 自动平仓 50% 锁定收益，并**自动上移防守止损线至建仓成本价**（锁定无风险持仓）；触及止损自动触发市价平仓双保险。**持仓看板支持动态止损可视化，包含初始止损对比、保本/锁利微章指示及红/绿风控状态颜色提醒。**
...
- **可视化配置与调试诊断**：内置配置面板，可直接在 GUI 界面中动态配置 OpenAI / DeepSeek / GLM API Key、自定义 Base URL、扫描频次、实盘 API Key / Secret 及推送策略，即时生效。**支持大模型请求参数实时回放与校验，在系统根目录下实时更新记录 `last_llm_request.json`，方便随时对发送给大模型的 Prompt 和原始 JSON 负载进行审计校验。**
